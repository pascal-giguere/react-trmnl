...
export function applyYogaStyle(node: YogaNode, style: YogaStyle): void {
  for (const styleProperty of Object.keys(style)) {
    try {
      applyYogaStyleProperty(node, style, styleProperty as keyof YogaStyle);
    } catch (error) {
      console.warn(`Failed to apply style property '${styleProperty}' to Yoga node: ${error}`);
    }
  }
}

function applyYogaStyleProperty(node: YogaNode, style: YogaStyle, property: keyof YogaStyle): void {
  switch (property) {
    case "alignContent":
    ...
    case "width":
      ...
      break;
    default:
      throw new Error(`Invalid Yoga style property: '${property}'`);
  }
}
